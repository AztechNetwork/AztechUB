class Strings(object):
    ...
    def about_tab_string(self):
        ...
        text += "I am Power made by Aztech.\n"
        ...
        text += "which are related to Aztech UserBot then just ask in\n"
        text += f"[support group](https://t.me/AzTechSupport)"

        return text


    def close_tab_string(self):
        text = "Welcome to Aztech.\n"
        text += "This is your Helpdex, Tap on open button to get more buttons,\n"
        text += "which will help you to understand  operate your userbot & assistant ( POWER )\n"
        text += "\n\n• Menu is closed"
    ...
    def ialive_tab_string(self):
        ...
        text += f"** Aztech:** `{self.userbot_version}`\n"
        text += f"** Python:** `{self.python_version}`\n"
        text += f"** Pyrogram:** `{self.pyrogram_version}`\n"
        text += f"** uptime:** `{self.uptime()}`\n"
