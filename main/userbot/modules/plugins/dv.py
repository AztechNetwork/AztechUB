...
async def pm_handler(_, m: Message):
    ...
    if app.long() == 1:
        ...
        if app.Pmpermit() is True:
            return await app.send_edit("PM Guard is already active !", text_type=["mono"], delme=4)

        done = app.setdv("PMPERMIT", "True")
        if done:
            await app.send_edit("PM Guard is now turned on.", text_type=["mono"], delme=4)
        else:
        ...
        if app.Pmpermit() is False:
            return await app.send_edit("PM Guard is already off !", text_type=["mono"], delme=4)

        done = app.deldv("PMPERMIT")
        if done:
            await app.send_edit("PM Guard is now turned off.", text_type=["mono"], delme=4)
        else:
            await app.send_edit("Failed to turn off PM Guard.", text_type=["mono"], delme=4)
