...
AZTECH_REPO = app.UPSTREAM_REPO


async def gen_chlog():
    changes = []
    last_updated = int(re.sub(r"[- + : \s]", "", str(app.heroku_app().updated_at))[:14])
    recent_updates = await app.GetRequest("https://api.github.com/repos/AztechNetwork/AztechUB/events")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # else create one
                origin = repo.create_remote(remote, AZTECH_REPO)
